...
#
# KernelSageModel: Derived from GptOssForCausalLM to demo custom model
# registration with a new attention kernel in vLLM.
...
class KernelSageTransformer(nn.Module):
    def __init__(
        self,
        *,
        vllm_config: VllmConfig,
...
class KernelSageModel(nn.Module, SupportsPP, SupportsEagle3, SupportsLoRA):
    """KernelSageModel: A custom model derived from GptOssForCausalLM
    demonstrating how to register a model with a new attention kernel in vLLM.
    """
    ...
    def __init__(
        ...
        self.model = KernelSageTransformer(
